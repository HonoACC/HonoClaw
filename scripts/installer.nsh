; HonoClaw Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting HonoClaw runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers HonoClaw.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; PowerShell path-based kill may miss processes if the old HonoClaw was installed
  ...
  ; "Can't modify HonoClaw's files" retry loop -> "HonoClaw 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "HonoClaw 无法关闭" dialog.
...
; "HonoClaw 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with HonoClaw's
  ...
  IfFileExists "$INSTDIR._stale_0\" 0 _ci_stale_cleaned
    ...
    ; E.g. $INSTDIR = D:\Apps\MyHonoClaw → glob = MyHonoClaw._stale_*
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to remove HonoClaw application data?$\r$\n$\r$\nThis will delete:$\r$\n  • AppData\Local\honoclaw (local app data)$\r$\n  • AppData\Roaming\honoclaw (roaming app data)$\r$\n$\r$\nYour .openclaw folder (configuration & skills) will be preserved.$\r$\nSelect 'No' to keep all data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering HonoClaw processes (and their child process trees) to
    ...
    RMDir /r "$LOCALAPPDATA\honoclaw"
    RMDir /r "$APPDATA\honoclaw"

    ; --- Retry: if directories still exist (locked files), wait and try again ---

    ; Check AppData\Local\honoclaw
    IfFileExists "$LOCALAPPDATA\honoclaw\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\honoclaw"
      IfFileExists "$LOCALAPPDATA\honoclaw\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\honoclaw"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\honoclaw
    IfFileExists "$APPDATA\honoclaw\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\honoclaw"
      IfFileExists "$APPDATA\honoclaw\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\honoclaw"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    StrCpy $R3 ""
    IfFileExists "$LOCALAPPDATA\honoclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\honoclaw"
    IfFileExists "$APPDATA\honoclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\honoclaw"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R3\AppData\Local\honoclaw"
    RMDir /r "$R3\AppData\Roaming\honoclaw"
