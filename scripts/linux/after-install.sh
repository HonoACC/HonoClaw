...
# Post-installation script for HonoClaw on Linux
...
# Create symbolic link for HonoClaw app binary
if [ -x /opt/HonoClaw/clawx ]; then
    ln -sf /opt/HonoClaw/clawx /usr/local/bin/clawx 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/HonoClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/HonoClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/HonoClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/HonoClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/honoclaw'
...
echo "HonoClaw has been installed successfully."
